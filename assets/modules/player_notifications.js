...
(function () {
   ...
   *
   *   "gameInfo" — notify enrolled players of pairings/tee times. Ungated
   *                (as of this pass — was previously blocked behind a
   *                readiness check; that gate is retired). "Send Tee
   *                Sheet" now always produces the richest content the
   *                game's current administration state supports — a full
   *                tee sheet once slotted, pairing groups once paired but
   *                not slotted, a flat roster list once enrolled but not
   *                paired — rather than refusing to send anything until
   *                slotting is complete. See initPlayerNotifications.php's
   *                tiered dispatch and game.gameInfoComplete, which
   *                drives the "Tee Sheet" vs "Tee Sheet*" subject/body
   *                framing below. Default tab: Game Players when the game
   *                has any enrolled players, else Favorites — same
   *                fallback "message" uses, since a real game can exist
   *                with zero players. All contactable enrolled players
   *                pre-selected when the Game Players tab is the default.
   */
  MA.notify.open = async function (options) {
    ...
    // All three intents are ungated — open immediately, load in place.
    // "gameInfo" was previously gated on readiness (isSlottingReady); that
    // gate is retired, since "Send Tee Sheet" now always produces
    // whatever content the game's current state supports rather than
    // refusing to send. "message" was never gated (an earlier design
    // considered isRosterReady, deliberately not enforced — admins can
    // message an empty game; readyForMessage is still returned by the API
    // for reference).
  ...
  // ── All three intents: no gate — open immediately, load in place ────
  ...
  function _openPanelWithData() {
    ...
    if (intent === "invite") {
      ...
      // "gameInfo" and "message" share this logic — a real game can exist
      // with zero enrolled players (hasGameContext alone doesn't tell us
      // that), and now that "gameInfo" is ungated too, it can open in
      // that state just as easily as "message" always could. Default to
      // Favorites in that case rather than opening on an empty Game
      // Players list.
  ...
  function _send(method) {
    ...
    if (game) {
      ...
      // Middle dot throughout (not em-dash) — matches the separator
      // already used inside the tee-sheet body content, so subject and
      // body read consistently rather than mixing punctuation styles.
      const subjectTail = [when, game.title, venue].filter(Boolean).join(" \u{b7} ");

      if (intent === "invite") {
        subject = subjectOverride || ("You're invited to play \u{b7} " + subjectTail);

        // No default body content — the subject line already carries the
        // invite framing (lead phrase, title, when, venue); restating it
        // here just duplicated the same text. The admin can add their own
        // note via bodyOverride; the link always appears regardless.
        body = (bodyOverride || "") + (bodyOverride ? "\n\n" : "") + "View or Register at " + link;

      } else if (intent === "message") {
        subject = subjectOverride || ("Message from your Game Admin \u{b7} " + subjectTail);
        ...
        // "Tee Sheet" vs "Tee Sheet*" — the asterisk signals the content
        // is partial (some tier below full slotting: pairing groups only,
        // or a flat roster list), driven by game.gameInfoComplete from
        // initPlayerNotifications.php's tiered dispatch. A complete tee
        // sheet gets no marker at all — identical to this feature's
        // original single-tier behavior. The matching "*Partial Tee
        // Sheet" line inside the body itself (when incomplete) is built
        // server-side, as part of game.gameInfoBody — not duplicated here.
        const leadLabel = game.gameInfoComplete ? "Tee Sheet" : "Tee Sheet*";
        subject = subjectOverride || (leadLabel + " \u{b7} " + subjectTail);

        // Prefer the caller's override, then the server-built tee-sheet
        // text (game.gameInfoBody, from ServiceGameRosterViews' three
        // view-builders + initPlayerNotifications.php's tiered dispatch —
        // identical regardless of which page triggered the send), then
        // bare link if neither is present (nobody enrolled at all).
