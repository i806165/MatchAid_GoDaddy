...
(function () {
  ...
  const wiz = {
    ...
    bbCountLock:        false,
    segments:           null,
    scoringSegments:    "1",   // PairPair only; "1" or "3" — normalized to "1" for PairField on save
    placementPoints:    null,  // JSON string or null; shape depends on dbGames_Competition (owned by module_definePlacementPoints.js)
    rotation:           null,
  ...
  function wizHydrateFromGame() {
    ...
    wiz.scoringSegments = String(g.dbGames_ScoringSegments || "1");
    wiz.placementPoints = g.dbGames_PlacementPoints ?? null;
  ...
  function wizRenderStep3() {
    ...
    // Placement Points — both competition types
  ...
  function openPlacementPointsConfigurator() {
    if (!window.MA?.definePlacementPoints?.open) {
